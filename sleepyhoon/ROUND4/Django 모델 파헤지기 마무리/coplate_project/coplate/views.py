...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        ...
        if user.is_authenticated:
            # 리뷰 중 내가 작성자의 follower인 케이스만 보여준다는 논리
...
class ReviewDetailView(DetailView): # 리뷰와 관련된 정보를 조회하는 뷰
    model = Review
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            context['liked_comments'] = Comment.objects.filter(review=review).filter(likes__user=user) # likes_user 안에 user가 포함되어 있으면 됨
        return context
...
class CommentCreateView(LoginAndVerificationRequiredMixin, CreateView):
    ...
    def get_success_url(self): # 성공하면 이동할 url 설정
...
class ProcessLikeView(LoginAndVerificationRequiredMixin,View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if not created:
            like.delete()
        return redirect(self.request.META['HTTP_REFERER']) # post 요청이 날아온 페이지로 돌아가기
